...
def compare_node(state: GraphState) -> GraphState:
    ...
    if isinstance(last, ToolMessage):
        ...
        response = get_llm(temperature=0.3).invoke([SystemMessage(content=COMPARE)] + history)
    else:
        # First pass — bind tools so Gemini calls get_candidates_summary
        response = get_llm(temperature=0.3).bind_tools(_TOOLS).invoke([SystemMessage(content=COMPARE)] + history)
